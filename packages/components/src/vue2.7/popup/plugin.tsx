...
import { getAttach, once, on, off } from "@td/adapter-utils";
...
import type { TNode, ClassName } from "@td/shared/interface";
...
const triggerType = (
  triggerProps: string
): Record<(typeof triggers)[number], boolean> =>
  triggers.reduce(
    (map, trigger) => ({
      ...map,
...
const Overlay = mixins(classPrefixMixins).extend({
    ...
    triggerEl: {
      validator(value) {
        if (!(value instanceof HTMLElement)) {
          log.warn(
            "Popup",
            `Invalid value for prop "triggerEl": expected an HTMLElement, but got ${typeof value}.`
          );
          return false;
        }
        return true;
  ...
  render(h): VNode {
    ...
    const { handleMouseLeave, handleMouseEnter, visibleState, hasTrigger } =
      this;
    const renderNode = h(
      "div",
      {
      ...
      [
        h(
          ...
          [
            content,
            this.showArrow &&
              h("div", { class: `${this.componentName}__arrow` }),
          ]
        ),
      ]
    );
    return visibleState ? (
      <transition
        slot="content"
        name={`${this.componentName}--animation`}
        appear
      >
        {renderNode}
      </transition>
...
export const createPopupPlugin: PluginMethod = (
  trigger,
  content,
  popupProps
) => {
...
export const PopupPlugin: PluginMethod & Vue.PluginObject<undefined> =
  createPopupPlugin as any;
