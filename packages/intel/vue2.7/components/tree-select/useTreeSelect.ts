...
import type { TreeKeysType, TreeOptionData } from '@td/shared/interface';
import type { TreeInstanceFunctions, TreeNodeValue, TreeProps } from '../tree';
...
import type { RemoveOptions, TdTreeSelectProps, TreeSelectValue } from './type';
...
export default function useTreeSelect(props: TdTreeSelectProps, context: SetupContext) {
  ...
  const singleActivated = computed(() => {
    if (props.multiple || !nodeInfo.value) {
      return [];
    }
    if (Array.isArray(nodeInfo.value)) {
      return [nodeInfo.value[0]?.[tKeys.value.value]];
    }
  ...
  const filterByText = computed<TreeProps['filter'] | undefined>(() => {
    if (props.onSearch || context.listeners.search) {
      return;
    }
  ...
  const inputChange: SelectInputProps['onInputChange'] = (value = '', ctx) => {
    if (value === innerInputValue.value) {
      return;
    }
