...
export default {
  before({ source, file }) {
    ...
    if (fileName && source.includes(':: BASE_DOC ::')) {
      ...
      const localeDocPath = path.posix.resolve(__dirname, '../../',  `node_modules/shared/_common/docs/web/api/${fileName}`);
      const defaultDocPath = path.posix.resolve(__dirname, '../../', `node_modules/shared/_common/docs/web/api/${componentName}.md`);
